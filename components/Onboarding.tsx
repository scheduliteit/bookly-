          ...
          <div className="max-w-md mx-auto bg-white p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6 animate-in fade-in duration-500">
            <div className="text-left">
              ...
              <input 
                ...
                className="mt-2 w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-lg font-bold focus:ring-2 focus:ring-brand-blue outline-none"
                value={name}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {categories.map(cat => (
                <button 
                  ...
                  className={`p-4 bg-white border-2 rounded-2xl flex flex-col items-center gap-2 transition-all ${category === cat.label ? 'border-brand-blue bg-brand-blue/5 text-brand-blue' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}
                >
                  <cat.icon size={20} />
