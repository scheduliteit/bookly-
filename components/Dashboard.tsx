    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-32 max-w-7xl mx-auto">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-100 pb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onAddEventType} className="px-8 py-2.5 bg-brand-blue text-white rounded-full font-bold text-sm hover:shadow-lg shadow-brand-blue/20 transition-all flex items-center gap-2">
      ...
      <div className="flex items-center gap-4 py-3 px-5 bg-white border border-slate-200 rounded-xl shadow-sm">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-400 font-medium">easybookly.com/book/{user.id}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {displayServices.length > 0 ? (
              ...
              <div className="p-6 flex-1">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="flex items-center gap-2">
                     <button onClick={onAddEventType} className="text-slate-400 hover:text-brand-blue p-1.5 rounded-lg hover:bg-slate-50 transition-all"><Settings size={18} /></button>
          ...
          <div className="col-span-full py-20 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center text-center gap-4">
             <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center text-slate-300 shadow-sm">
               <Plus size={32} />
             </div>
             <div>
               <h3 className="font-bold text-brand-dark">No event types yet</h3>
               <p className="text-sm text-slate-500">Create your first event type to start taking bookings.</p>
             </div>
             <button onClick={onAddEventType} className="mt-2 px-8 py-2.5 bg-brand-blue text-white rounded-full font-bold text-sm shadow-md">Create Event Type</button>
          </div>
        )}
      </div>
    </div>
